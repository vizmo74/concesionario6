...
type RestOf<T extends IVendedor | NewVendedor> = Omit<T, 'antiguedad'> & {
...
export class VendedorService {
  ...
  protected convertDateFromClient<T extends IVendedor | NewVendedor | PartialUpdateVendedor>(vendedor: T): RestOf<T> {
    return {
      ...vendedor,
      antiguedad: vendedor.antiguedad?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restVendedor: RestVendedor): IVendedor {
    return {
      ...restVendedor,
      antiguedad: restVendedor.antiguedad ? dayjs(restVendedor.antiguedad) : undefined,
    };
  }
